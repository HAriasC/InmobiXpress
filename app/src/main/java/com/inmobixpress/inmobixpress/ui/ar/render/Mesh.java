 ...
 *
 * <p>To render the mesh, use {@link ARRender#draw()}.
 */
public class Mesh implements Closeable {
   ...
   * (location 2, vec3).
   */
  public static Mesh createFromAsset(ARRender render, String assetFileName) throws IOException {
    try (InputStream inputStream = render.getAssets().open(assetFileName)) {
      ...
      float[] vertex = ObjData.getVerticesArray(obj);
      ...
      return new Mesh(render, PrimitiveMode.TRIANGLES, indexBuffer, vertexBuffers, vertex);
    }
  }
  ...
  public int getNumberOfVertices() {
    return vertexBuffers[0].getNumberOfVertices();
  }
